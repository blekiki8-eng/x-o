...
async def handle_bowling_dice(m: types.Message):
    # Перевіряємо, чи це саме боулінг
    if m.dice.emoji != "🎳":
        return

    uid = m.from_user.id
    # Шукаємо активну гру, де бере участь цей гравець
    g = await games_col.find_one({"status": "playing", "$or": [{"creator_id": uid}, {"opponent_id": uid}]})
    
    if not g:
        return # Гравець просто кинув боулінг поза грою

    if g['turn'] != uid:
        return await m.reply("Зараз не ваш хід! Зачекайте суперника.")
    ...
    if uid == cid:
        ...
        if rounds >= g['mode']:
            ...
            res_text_base = f"Результат:\nВи: {{your}}\nСуперник: {{opp}}"
            
            if total_c > total_o:
                await users_col.update_one({"_id": cid}, {"$inc": {"balance": win_sum}})
                await bot.send_message(cid, f"{res_text_base.format(your=total_c, opp=total_o)}\n\nВітаю ви виграли {win_sum} 💎")
                await bot.send_message(oid, f"{res_text_base.format(your=total_o, opp=total_c)}\n\nНажаль ви програли")
                ...
                await bot.send_message(cid, f"Результат: {total_c} - {total_o}\n🤝 Нічия! Ставки повернуто.")
                await bot.send_message(oid, f"Результат: {total_o} - {total_c}\n🤝 Нічия! Ставки повернуто.")
            
            await games_col.update_one({"game_id": gid}, {"$set": {"status": "finished"}})
        else:
            # Наступний раунд
            await games_col.update_one({"game_id": gid}, {"$set": {"o_score": new_score, "turn": cid, "rounds_done": rounds}})
...
async def withdr_fin(m: types.Message, state: FSMContext):
    ...
    await m.answer("✅ Заявку на вивід прийнято. Очікуйте виплати.", reply_markup=main_menu(m.from_user.id))
    await state.finish()
...
async def start_cmd(m: types.Message, state: FSMContext):
    ...
    args = m.get_args()
    uid = m.from_user.id
    await get_u(uid, m.from_user.full_name)
    ...
    if args and args.startswith("game_"):
        ...
        if g and g['creator_id'] != uid:
            ...
            await bot.send_message(uid, f"🎮 Гра почалася! Режим: {g['mode']} раундів.")
            await bot.send_message(g['creator_id'], f"🎮 Суперник приєднався! Починаємо.")
...
async def bowling_menu(m: types.Message):
...
async def set_mode(m: types.Message, state: FSMContext):
    ...
    if m.text in ["1 гра", "5 ігор"]:
        await state.update_data(m=1 if "1" in m.text else 5)
