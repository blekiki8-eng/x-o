...
async def cmd_start(m: types.Message, state: FSMContext):
    ...
    # Реєстрація або отримання профілю
    ref_id = None
    ...
    if len(args) > 1 and args[1].startswith("game_"):
        ...
        if g['creator_id'] == user_id: 
            return await m.answer("❌ Це ваша гра. Очікуйте суперника.", reply_markup=main_kb())
        if u['balance'] < g['bet']: 
            return await m.answer(f"❌ Недостатньо балансу. Треба {g['bet']} 💎", reply_markup=main_kb())
        
        # Списання ставки та початок гри
...
async def g_create(m: types.Message, state: FSMContext):
    try:
        ...
        if u['balance'] < bet: return await m.answer("❌ Немає балансу")
...
async def dice_handler(m: types.Message):
    ...
    if len(upd['c_score']) == upd['rounds_total'] and len(upd['o_score']) == upd['rounds_total']:
        ...
        await m.answer(f"Випало: {val}. Чекаємо суперника...")
...
async def dep_r(m: types.Message, state: FSMContext):
    ...
    await m.answer("✅ Чек на розгляді!", reply_markup=main_kb()); await state.clear()
...
async def draw_1(m: types.Message, state: FSMContext):
    try:
        ...
        await state.update_data(amt=amt)
        await m.answer("Дані для виводу (IBAN, ПІБ, ІПН):")
...
async def adm_tr(cb: types.CallbackQuery):
    ...
    msg = "🏆 **Топ рефералів:**\n" + "\n".join([f"{u['_id']}: {u.get('referals_count', 0)}" for u in top])
...
async def adm_tt(cb: types.CallbackQuery):
    ...
    msg = "📊 **Топ оборотів:**\n" + "\n".join([f"{u['_id']}: {u.get('turnover', 0.0):.2f}" for u in top])
    await cb.message.answer(msg)
...
async def fin_handle(cb: types.CallbackQuery):
    ...
    if act == "fok":
        ...
        await bot.send_message(f['uid'], "❌ Відхилено.")
    elif act == "dok":
        await bot.send_message(f['uid'], "✅ Виплачено!")
    elif act == "dno":
        await users_col.update_one({"_id": f['uid']}, {"$inc": {"balance": f['amt']}})
