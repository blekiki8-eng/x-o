...
# --- КОМАНДА СТАРТ (ОБРОБКА ГРИ ТА РЕФЕРАЛІВ) ---
...
async def start_cmd(m: types.Message, state: FSMContext):
    ...
    # 1. Спершу створюємо або отримуємо юзера
    u = await users_col.find_one({"_id": uid})
    if not u:
        inviter = None
    ...
    # 2. Обробка входу в гру
    if args and args.startswith("game_"):
        gid = args.replace("game_", "")
        g = await games_col.find_one({"game_id": gid, "status": "waiting"})
        
        if not g:
            return await m.answer("❌ Гра не знайдена або вже почалась.", reply_markup=main_menu(uid))
        
        if g['creator_id'] == uid:
            return await m.answer("⏳ Це ваша гра. Очікуйте суперника.", reply_markup=main_menu(uid))

        if u['balance'] < g['bet']:
            # Якщо грошей нема, скасовуємо гру і повертаємо ставку творцю
            await users_col.update_one({"_id": g['creator_id']}, {"$inc": {"balance": g['bet']}})
            await games_col.delete_one({"game_id": gid})
            await bot.send_message(g['creator_id'], "❌ У суперника мало коштів. Гру скасовано, ставку повернуто.")
            return await m.answer("❌ Недостатньо 💎 для входу.", reply_markup=main_menu(uid))
        ...
        await m.answer("🎮 Гра почалася! Твій символ ⭕️. Хід суперника.", reply_markup=kb)
        return

    await m.answer(f"Вітаємо, {m.from_user.first_name}!", reply_markup=main_menu(uid))

# --- АДМІН-ПАНЕЛЬ (СПИСОК ГРАВЦІВ) ---
@dp.message_handler(lambda m: m.text == "🛡 Панель адміна", state="*")
async def admin_main(m: types.Message, state: FSMContext):
    if m.from_user.id != ADMIN_ID: return
    await state.finish()
    kb = InlineKeyboardMarkup().add(InlineKeyboardButton("👥 Список гравців", callback_data="adm_users"))
    await m.answer("🛡 Панель керування:", reply_markup=kb)

@dp.callback_query_handler(lambda c: c.data == "adm_users", state="*")
async def adm_list_users(c: types.CallbackQuery):
    users = await users_col.find().limit(50).to_list(length=50) # Перші 50 для прикладу
    kb = InlineKeyboardMarkup(row_width=1)
    for u in users:
        kb.add(InlineKeyboardButton(f"{u['nickname']} (ID: {u['_id']})", callback_data=f"info:{u['_id']}"))
    await bot.send_message(c.from_user.id, "Оберіть гравця для статистики:", reply_markup=kb)

@dp.callback_query_handler(lambda c: c.data.startswith("info:"), state="*")
async def adm_user_info(c: types.CallbackQuery):
    uid = int(c.data.split(":")[1])
    u = await users_col.find_one({"_id": uid})
    if not u: return await c.answer("Не знайдено")
    
    text = (f"📊 Статистика гравця `{uid}`\n\n"
            f"👤 Нік: {u['nickname']}\n"
            f"💰 Баланс: {u['balance']} 💎\n"
            f"📥 Поповнено всього: {u['total_deposited']} 💎\n"
            f"🤝 Запросив: {u['referrals_count']} людей\n"
            f"🔗 Його запросив: {u['invited_by'] if u['invited_by'] else 'Ніхто'}")
    await bot.send_message(c.from_user.id, text, parse_mode="Markdown")

# --- ПРОФІЛЬ ТА РЕФЕРАЛКА (ЯК ТИ ПРОСИВ) ---
...
async def refer(m: types.Message, state: FSMContext):
    await state.finish()
    bot_un = (await bot.get_me()).username
    link = f"https://t.me/{bot_un}?start=ref_{m.from_user.id}"
    text = (f"Вітаю це реферальна система\nВаше посилання: `{link}`\n\n"
            f"Система діє так:\nЯкщо ви запросили свого друга в гру то ваш реферал коли виграє то ви получаєте на свій баланс 0.01%.")
    await m.answer(text, parse_mode="Markdown")
...
async def game_move(c: types.CallbackQuery):
    ...
    if win:
        ...
        if win == "draw":
            ...
            # Реф-бонус
            win_data = await users_col.find_one({"_id": winner})
            if win_data.get('invited_by'):
                bonus = win_sum * 0.0001
                await users_col.update_one({"_id": win_data['invited_by']}, {"$inc": {"balance": bonus}})
...
async def game_1(m: types.Message, state: FSMContext):
...
async def adm_res(c: types.CallbackQuery):
    if c.from_user.id != ADMIN_ID: return
    p = c.data.split(":"); uid = int(p[1])
    if "ok" in p[0]:
        amt = float(p[2])
        await users_col.update_one({"_id": uid}, {"$inc": {"balance": amt, "total_deposited": amt}})
        await bot.send_message(uid, f"✅ Схвалено! +{amt} 💎")
    else: await bot.send_message(uid, "❌ Відхилено.")
