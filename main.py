...
async def choose_rounds(m: types.Message, state: FSMContext):
    ...
    await m.answer(f"Обрано {g_type}. Кількість раундів:", reply_markup=rounds_kb())
...
async def join_game_logic(m, gid):
    ...
    if u2['balance'] < g['bet']: return await m.answer("❌ Недостатньо балансу.")
...
async def handle_dice(m: types.Message):
    ...
    if len(upd['c_score']) == upd['rounds_total'] and len(upd['o_score']) == upd['rounds_total']:
        ...
        await bot.send_message(next_t, f"🎰 Суперник вибив **{m.dice.value}**!\n✨ **Ваш хід!**", parse_mode="Markdown")
...
async def dep_receipt(m: types.Message, state: FSMContext):
    ...
    await bot.send_message(ADMIN_ID, f"🔔 Поповнення {data['amt']} 💎 від `{m.from_user.id}`", parse_mode="Markdown")
...
async def with_start(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.answer("📤 Сума виводу (мін 4.0):", reply_markup=cancel_kb())
...
async def with_amt(m: types.Message, state: FSMContext):
    try:
        amt = float(m.text.replace(",", "."))
        u = await get_u(m.from_user.id)
        if amt < 4.0 or u['balance'] < amt: return await m.answer("❌ Недостатньо коштів/ліміт")
...
async def adm_with(cb: types.CallbackQuery):
    ...
    if act == "ok":
        ...
        await bot.send_message(trans['user_id'], f"✅ Виплату {trans['amount']} 💎 виконано!")
        await cb.message.edit_text(cb.message.text + "\n\n✅ **ВИКОНАНО**")
    else:
        await finance_col.update_one({"_id": t_id}, {"$set": {"status": "w_rejected"}})
        await users_col.update_one({"_id": trans['user_id']}, {"$inc": {"balance": trans['amount']}})
        await bot.send_message(trans['user_id'], f"❌ Вивід відхилено. Кошти повернуто.")
        await cb.message.edit_text(cb.message.text + "\n\n❌ **ВІДХИЛЕНО**")
...
async def profile_cmd(m: types.Message):
    u = await get_u(m.from_user.id)
    await m.answer(f"👤 **Профіль**\n💰 Баланс: `{u['balance']:.2f}`\n🎳 Ігор: `{u['games_played']}`", parse_mode="Markdown")
...
async def bal_cmd(m: types.Message):
    ...
    await m.answer(f"💰 Баланс: `{u['balance']:.2f}` 💎", reply_markup=kb)
