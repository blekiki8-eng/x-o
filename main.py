...
async def referal_cmd(m: types.Message):
    ...
    text = (
        ...
        f"Якщо у гравця немає реферера, комісія 2% йде на розвиток проекту.\n\n"
...
async def balance_menu(m: types.Message):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="📤 Вивести", callback_data="draw")]
    ])
...
async def dep_a(m: types.Message, state: FSMContext):
    try:
        ...
        await m.answer(f"Заявка:\nСума: {amt} 💎\nОплата: {pay:.2f} ГРН\nРеквізити: `5355 2800 2890 2177`\n\nНадішліть ФОТО чеку!", reply_markup=cancel_kb(), parse_mode="Markdown")
...
async def draw_s(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.answer("💎 Введіть суму виведення (мін. 4.50):", reply_markup=cancel_kb())
...
async def draw_1(m: types.Message, state: FSMContext):
    try:
        ...
        await state.update_data(amt=amt)
        await m.answer("Вкажіть дані (Карта, ПІБ):", reply_markup=cancel_kb())
...
async def fin_handle(cb: types.CallbackQuery):
    ...
    if act == "fok":
        ...
        await bot.send_message(f['uid'], f"✅ +{f['amt']} 💎 нараховано!")
        if cb.message.photo: await cb.message.edit_caption(caption=cb.message.caption + "\n\n✅ Схвалено")
    elif act == "fno":
        await bot.send_message(f['uid'], "❌ Депозит відхилено.")
        if cb.message.photo: await cb.message.edit_caption(caption=cb.message.caption + "\n\n❌ Відхилено")
    elif act == "dok":
...
async def games_menu(m: types.Message):
    ...
    await m.answer("Режим гри:", reply_markup=kb)
...
async def g_mode(m: types.Message, state: FSMContext):
    ...
    await m.answer("Раунди:", reply_markup=ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="1"), KeyboardButton(text="5")], [KeyboardButton(text="❌ Скасувати")]], resize_keyboard=True))
...
async def dice_handler(m: types.Message):
    ...
    if len(upd['c_score']) == upd['rounds_total'] and len(upd['o_score']) == upd['rounds_total']:
        ...
        await m.answer(f"Випало: {m.dice.value}. Хід суперника...")
...
async def finish_game(g):
    ...
    for uid in [g['creator_id'], g['opponent_id']]:
        ...
        if s1 == s2:
            ...
            raw_win = g['bet'] * 2
            u = await users_col.find_one({"_id": uid})
            fee_p = 0.01 if u.get("referrer") else 0.02
            if u.get("referrer"):
                await users_col.update_one({"_id": u['referrer']}, {"$inc": {"balance": raw_win * 0.01}})
            win_net = raw_win - (raw_win * fee_p)
            await users_col.update_one({"_id": uid}, {"$inc": {"balance": win_net, "turnover": g['bet'], "games_played": 1}})
            await stats_col.update_one({"_id": "global"}, {"$inc": {"admin_balance": raw_win * fee_p}}, upsert=True)
            await bot.send_message(uid, res_text + f"Вітаю з перемогою +{win_net:.2f} 💎")
        else:
...
async def settings_panel(m: types.Message):
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🆘 Support", url="https://t.me/vex0o0")]])
    if m.from_user.id == ADMIN_ID:
        res = await stats_col.find_one({"_id": "global"})
        bal = res.get("admin_balance", 0) if res else 0
        await m.answer(f"⚙️ Налаштування\n💰 Адмін-баланс: {bal:.2f} 💎", reply_markup=kb)
    else: await m.answer("⚙️ Налаштування:", reply_markup=kb)
