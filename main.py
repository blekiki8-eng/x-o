...
async def profile_handler(m: types.Message):
    ...
    text = (f"👤 **Профіль**\n\n"
            ...
            f"📥 Всього поповнено: {round(u.get('total_deposited', 0.0), 2)} 💎\n"
            f"🤝 Рефералів запрошено: {u.get('referrals_count', 0)}")
...
async def dep_amt(m: types.Message, state: FSMContext):
    try:
        ...
        if amt < 0.5: return await m.answer("Мінімальна сума 0.5 💎")
...
async def dep_receipt(m: types.Message, state: FSMContext):
    ...
    await bot.send_photo(ADMIN_ID, m.photo[-1].file_id, caption=f"📥 Депозит {d['a']} 💎\nID: {m.from_user.id}", reply_markup=kb)
    await m.answer("✅ Квитанцію надіслано. Чекайте підтвердження.", reply_markup=main_menu(m.from_user.id))
    await state.finish()

# --- ЛОГІКА ХОДУ ТА РЕФЕРАЛЬНІ БОНУСИ ---
...
async def process_win(gid, winner_char_or_id, timeout=False, board=None):
    ...
    if timeout:
        ...
        res_text = "🏆 Перемога по таймеру! (Суперник не походив)"
        ...
        await bot.send_message(g['creator_id'], res_c); await bot.send_message(g['opponent_id'], res_o)
        return
    else:
    ...
    if winner_u.get("invited_by"):
        ref_bonus = round(win_sum * 0.0001, 6) # 0.01%
        await users_col.update_one({"_id": winner_u["invited_by"]}, {"$inc": {"balance": ref_bonus}})
        try: await bot.send_message(winner_u["invited_by"], f"📈 Реферальний бонус: +{ref_bonus} 💎 від виграшу друга!")
        except: pass

    kb = get_board_markup(gid, board if board else g['board'], g['size'])
    try:
        await bot.edit_message_text(res_c if not timeout else res_text, g['creator_id'], g['creator_msg_id'], reply_markup=kb)
        await bot.edit_message_text(res_o if not timeout else res_text, g['opponent_id'], g['opponent_msg_id'], reply_markup=kb)
    except: pass
...
async def admin_ok(c: types.CallbackQuery):
    ...
    await bot.send_message(int(uid), f"✅ Ваш баланс поповнено на {amt} 💎!")
