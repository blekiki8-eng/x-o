    ...
    if args:
        if args.startswith("game_"):
            ...
            if g and g['creator_id'] != uid:
                ...
                opp_m = await m.answer("🎮 Гра почалася! Твій хід наступний (⭕️)", reply_markup=get_board_markup(gid, g['board'], g['size']))
                await games_col.update_one({"game_id": gid}, {"$set": {"opponent_id": uid, "status": "playing", "turn": g['creator_id'], "opponent_msg_id": opp_m.message_id, "last_move_time": time.time()}})
                try:
...
async def dep_receipt(m: types.Message, state: FSMContext):
    ...
    await bot.send_photo(ADMIN_ID, m.photo[-1].file_id, caption=f"📥 Поповнення {d['a']} 💎\nID: {m.from_user.id}", reply_markup=kb)
...
async def withdr_start(c: types.CallbackQuery):
    ...
    await bot.send_message(c.from_user.id, "Введіть суму для виводу 💎:", reply_markup=cancel_kb())
...
async def withdr_amt(m: types.Message, state: FSMContext):
    ...
    try:
        ...
        await state.update_data(a=amt)
        await m.answer("Введіть номер вашої карти для виплати:", reply_markup=cancel_kb())
...
async def withdr_fin(m: types.Message, state: FSMContext):
    ...
    await m.answer("✅ Заявку на вивід прийнято. Очікуйте виплати протягом 24 год.", reply_markup=main_menu(m.from_user.id))
    await state.finish()
...
async def game_move(c: types.CallbackQuery):
    ...
    await games_col.update_one({"game_id": gid}, {"$set": {"board": new_board, "turn": next_p, "last_move_time": time.time()}})
    win = check_win(new_board, g['size'])
    if win: await process_win(gid, win, board=new_board)
    else:
        kb = get_board_markup(gid, new_board, g['size'])
        await bot.edit_message_text("⏳ Чекаємо хід...", c.from_user.id, c.message.message_id, reply_markup=kb)
...
async def process_win(gid, winner_char_or_id, timeout=False, board=None):
    ...
    if timeout:
        ...
        u = await users_col.find_one({"_id": winner_id})
        if u.get("invited_by"):
            bonus = round(win_sum * 0.0001, 6)
            await users_col.update_one({"_id": u["invited_by"]}, {"$inc": {"balance": bonus}})
