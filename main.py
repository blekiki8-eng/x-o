...
async def dep_amt(m: types.Message, state: FSMContext):
    try:
        ...
        text = (f"📊 **Заявка:**\n💰 Сума: `{amt:.2f} 💎`\n💵 Оплата: `{total_uah:.2f} ГРН` (з комісією 5%)\n"
                f"💳 **Mono:** `5355 2800 2890 2177`\n\nНадішліть чек фотографією:")
...
async def dep_final(m: types.Message, state: FSMContext):
    ...
    await bot.send_message(ADMIN_ID, f"🔔 Чек {data['amt']} від {uid}")
    ...
    await m.answer("✅ Надіслано на перевірку!", reply_markup=main_kb()); await state.clear()
...
async def with_start(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.answer("📤 Введіть суму виводу (мін 4.00 💎):", reply_markup=cancel_kb())
...
async def with_amt(m: types.Message, state: FSMContext):
    try:
        amt = float(m.text.replace(",", "."))
        u = await get_u(m.from_user.id)
        if amt < 4.0 or u['balance'] < amt: return await m.answer("❌ Недостатньо балансу/ліміт")
        await state.update_data(w_amt=amt)
        await m.answer("💳 Введіть реквізити (Карта та ПІБ):", reply_markup=cancel_kb())
...
async def with_final(m: types.Message, state: FSMContext):
    ...
    await bot.send_message(ADMIN_ID, f"📤 ВИВІД {data['w_amt']} 💎\nЮзер: {m.from_user.id}\nДані: {m.text}")
...
async def adm_process(cb: types.CallbackQuery):
    ...
    if act == "ok":
        ...
        await bot.send_message(uid, "✅ Поповнено!"); await cb.message.edit_text("OK ✅")
    else: await bot.send_message(uid, "❌ Відхилено"); await cb.message.edit_text("NO ❌")
